  ...
  return (
    <div
      className={`
        ...
        animate-fade-up
        ${task.status === 'need_2fa' ? 'animate-shake' : ''}
      `}
    >
      {/* Header row */}
      ...
      {/* Terminal log — visible during active states */}
      {(task.status === 'pending' || task.status === 'processing' ||
        task.status === 'need_2fa' || task.status === 'success' ||
        task.status === 'failed') && (
        <TerminalLog status={task.status} taskId={task.id} />
      )}
