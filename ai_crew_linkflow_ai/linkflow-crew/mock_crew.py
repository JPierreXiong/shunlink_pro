...
class MockBacklinkCrew:
    ...
    def run(self) -> MockCrewResult:
        ...
        # Allow forcing outcome via env var for testing
        # MOCK_OUTCOME=success | need_2fa | failed | random (default)
        forced_outcome = os.getenv("MOCK_OUTCOME", "random").lower()

        # Simulate AI thinking + browser work
        delay = random.uniform(2, 5) if forced_outcome != "random" else random.uniform(15, 30)
        logger.info(f"[MOCK] Simulating {delay:.0f}s of AI + browser work...")
        time.sleep(delay)

        if forced_outcome == "random":
        ...
        if outcome == "success":
            ...
            result = MockCrewResult(
                status="failed",
                error_message="[MOCK] Simulated permanent failure",
                raw_output=f"[MOCK] Failed for task {task_id}",
            )
            logger.info(f"[MOCK] FAILED -> {result.error_message}")
