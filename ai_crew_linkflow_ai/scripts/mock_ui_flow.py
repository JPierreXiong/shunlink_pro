...
def resolve_field_names() -> dict:
    """Resolve schema differences between older/newer task tables."""
    c = conn()
    ...
    twofa_prompt = "two_fa_prompt" if "two_fa_prompt" in cols else "twofa_prompt"
    live_url = "live_url" if "live_url" in cols else "live_link_url"

    missing = [k for k in [twofa_code, twofa_prompt, live_url] if k not in cols]
    if missing:
        print(f"[ERROR] Missing required columns in backlink_tasks: {missing}")
        sys.exit(1)

    print(
        f"[SCHEMA] using fields: {twofa_code=}, {twofa_prompt=}, {live_url=}"
    )
    return {
        "twofa_code": twofa_code,
        "twofa_prompt": twofa_prompt,
        "live_url": live_url,
    }
...
def set_status(task_id: str, status: str, extra: dict = None):
    ...
    c.commit()
    c.close()
    print(f"  [DB] task {task_id[:8]}... → status='{status}'" + (f" | {extra}" if extra else ""))
...
""")
    ...
    set_status(task_id, "need_2fa", {
        fields["twofa_prompt"]: "[MOCK] Platform requires 2FA — enter the 6-digit code from your authenticator app."
    })

    print("\n  Go to your dashboard and enter any 6-digit code in the card.")
    print("  Waiting for 2FA code to appear in DB (polling every 3s)...")

    for _ in range(60):  # max 3 minutes
        task = get_task(task_id)
        code = task.get(fields["twofa_code"]) or ""
    ...
    set_status(task_id, "success", {
        fields["live_url"]: "https://mock-blogger.example.com/p/linkflow-test",
