        ...
        {hero.announcement && (
          <motion.div {...createFadeInVariant(0)}>
            <Link
              ...
              className="hover:bg-muted/80 bg-muted/60 dark:bg-muted/40 group mx-auto mb-8 flex w-fit items-center gap-2 rounded-lg border border-border/50 px-3 py-1.5 shadow-sm transition-all duration-300 hover:shadow-md"
            >
              {/* Extract and display lock icon if emoji is present */}
              {hero.announcement.title?.includes('🔒') ? (
                <>
                  <Lock size={14} className="text-foreground/80 flex-shrink-0" />
                  <span className="text-foreground text-sm font-medium">
                    {hero.announcement.title.replace('🔒', '').trim()}
                  </span>
                </>
              ) : (
                <span className="text-foreground text-sm font-medium">
