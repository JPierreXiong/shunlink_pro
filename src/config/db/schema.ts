...
export const alertHistory = pgTable(
  'alert_history',
  {
    ...
    channels: text('channels').notNull(), // JSON array: ["email", "telegram"]
    data: jsonb('data'), // Alert context data
    errorMessage: text('error_message'),
    createdAt: timestamp('created_at').defaultNow().notNull(),
  },
  (table) => [
    // Query history for a site
    index('idx_alert_history_site').on(table.siteId),
    // Query history for a rule
...
// ============================================
// LinkFlow AI - Backlink Platform Tables
// ============================================

export const backlinkPlatforms = pgTable(
  'backlink_platforms',
  {
    id: text('id').primaryKey(),
    name: text('name').notNull(),
    ...
    sensitivity: text('sensitivity').default('medium'), // low, medium, high
    notes: text('notes'), // internal notes e.g. "Reddit is sensitive to account age"
    createdAt: timestamp('created_at').defaultNow().notNull(),
    updatedAt: timestamp('updated_at')
      .$onUpdate(() => new Date())
      .notNull(),
  ...
  (table) => [
    index('idx_backlink_platforms_active').on(table.isActive),
    index('idx_backlink_platforms_slug').on(table.slug),
  ]
);

export const backlinkTasks = pgTable(
  'backlink_tasks',
  {
    id: text('id').primaryKey(),
    userId: text('user_id')
    ...
    platformId: text('platform_id')
      .references(() => backlinkPlatforms.id),
    targetUrl: text('target_url').notNull(),
    anchorText: text('anchor_text').notNull(),
    agentPersona: text('agent_persona').default('professional'), // professional, social
    aiOptimize: boolean('ai_optimize').default(true), // AI content optimization
    // Status flow: pending -> processing -> need_2fa -> success / failed
    ...
    // Debug/audit fields
    browserFingerprint: jsonb('browser_fingerprint'), // UA, resolution, etc.
    proxyIp: text('proxy_ip'),
    errorMessage: text('error_message'),
    agentLog: text('agent_log'), // raw agent output log
    createdAt: timestamp('created_at').defaultNow().notNull(),
    updatedAt: timestamp('updated_at')
      .$onUpdate(() => new Date())
      .notNull(),
  ...
  (table) => [
    index('idx_backlink_tasks_user').on(table.userId),
    index('idx_backlink_tasks_status').on(table.status),
    index('idx_backlink_tasks_user_status').on(table.userId, table.status),
    index('idx_backlink_tasks_created').on(table.createdAt),
  ]
);
