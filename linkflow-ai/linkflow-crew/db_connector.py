...
def mark_failed_and_refund(conn, task_id: str, user_id: str, error_msg: str) -> None:
    ...
    with conn.cursor() as cur:
        ...
        """, (error_msg, task_id))
        cur.execute(
            "UPDATE users SET credit_balance = credit_balance + 1 WHERE id = %s",
            (user_id,)
        )
    conn.commit()
...
def increment_platform_failure(conn, platform_id: int) -> int:
    """
    ...
    if row:
        data = dict(row)
        new_count = data["failure_count"]
        if not data["is_active"]:
            logger.warning(
                f"Platform {platform_id} auto-disabled after {new_count} consecutive failures. "
...
def reset_platform_failure(conn, platform_id: int) -> None:
    """
